...
function getEscudo(time) {
  return escudos[time] || "https://via.placeholder.com/40/0033a0/ffffff?text=Time";
}

// Estrelas leves
function createStars() {
  var starsContainer = document.querySelector(".stars-background");
  if (!starsContainer) return;
  starsContainer.innerHTML = "";
  var starCount = window.innerWidth < 768 ? 12 : 24;
...
async function loadData() {
  ...
  resultadosDiv.innerHTML = '<div class="loading"><div class="loading-spinner"></div><p>Carregando resultados...</p></div>';
  if (btnAtualizar) {
  ...
  try {
    ...
    for (var i = 1; i < data.values.length; i++) {
      ...
      allResults.push({
        date: row[0] || "",
        team1: row[1] || "",
        score: row[2] ? row[2].split(/(?=[A-Za-z])/)[0].trim() : "",
        team2: row[3] || "",
        status: row[4] || "",
        competition: row[5] || ""
      });
    ...
    for (var j = 0; j < allResults.length; j++) {
      var comp = allResults[j].competition;
      if (comp && competitions.indexOf(comp) === -1) competitions.push(comp);
    }
    ...
    setTimeout(function () { resultadosDiv.classList.remove("updated"); }, 1000);
  } catch (error) {
...
function displayResults(results, selectedCompetition, competitions) {
  ...
  var filteredResults = selectedCompetition
    ? results.filter(function (r) { return r.competition === selectedCompetition; })
    : results;
  ...
  for (var i = 0; i < competitions.length; i++) {
    html += '<option value="' + competitions[i] + '"' +
      (selectedCompetition === competitions[i] ? " selected" : "") +
      '>' + competitions[i] + '</option>';
  }
  html += '</select></div></div>' +
    '<div class="results-table-container">' +
    '<table><thead><tr>' +
    '<th>Data</th><th>Jogo</th><th>Resultado</th><th>Competição</th>' +
    '</tr></thead><tbody>';
  if (filteredResults.length === 0) {
    html += '<tr><td colspan="4" style="text-align: center; padding: 2rem;">Nenhum resultado encontrado</td></tr>';
  } else {
    for (var j = 0; j < filteredResults.length; j++) {
      var result = filteredResults[j];
      var matchResult = getMatchResult(result);
      var scoreParts = result.score.split(/(?=[A-Za-z])/);
      var score = scoreParts[0] ? scoreParts[0].trim() : "";
      var gameType = scoreParts.length > 1 ? scoreParts.slice(1).join("") : "";
      var isHome = result.team1.indexOf("Cruzeiro") !== -1;
      var resultIcon = "";
      if (matchResult === "vitoria") {
        resultIcon = '<span class="res-ico vitoria">V</span>';
      } else if (matchResult === "empate") {
        resultIcon = '<span class="res-ico empate">E</span>';
      } else if (matchResult === "derrota") {
        resultIcon = '<span class="res-ico derrota">D</span>';
      }
      html += '<tr class="result-row">' +
        '<td class="data">' + formatDate(result.date) + '</td>' +
        '<td>' +
        '<div class="jogo">' +
        '<img src="' + getEscudo(result.team1) + '" alt="' + result.team1 + '" class="escudo-time">' +
        '<span class="' + (isHome ? "time-casa" : "") + '">' + result.team1 + '</span>' +
        ' x ' +
        '<img src="' + getEscudo(result.team2) + '" alt="' + result.team2 + '" class="escudo-time">' +
        '<span class="' + (!isHome ? "time-casa" : "") + '">' + result.team2 + '</span>' +
        '</div></td>' +
        '<td><span class="resultado ' + matchResult + '">' + resultIcon + ' ' + score + '</span></td>' +
        '<td class="campeonato">' + result.competition +
        (gameType ? '<span class="tipo-jogo">' + gameType + '</span>' : "") +
        '</td></tr>';
    }
  }
...
function getMatchResult(result) {
  if (!result.score || result.score.indexOf("-") === -1) return "";
  var goals = result.score.split("-");
  var goals1 = parseInt(goals[0], 10);
  var goals2 = parseInt(goals[1], 10);
  var isHome = result.team1.indexOf("Cruzeiro") !== -1;
  if (isHome) {
