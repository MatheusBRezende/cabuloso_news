...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Atualiza título e informações do jogo
  atualizarInformacoesJogo(jogoAoVivo);
  ...
  const narrativaContainer = document.getElementById("narrativa-jogo");
  if (narrativaContainer) {
    narrativaContainer.addEventListener("scroll", handleScroll);
  }
...
function handleScroll() {
  ...
  const botaoExistente = document.querySelector(".voltar-baixo-narrativa");
...
const padraoGol = /^(\d{1,3}'\+\d+'|\d{1,3}')?\s*(?:Fim do (?:Jogo|segundo tempo|primeiro tempo|Intervalo),?\s*)?([^,0-9]+?)\s*(\d+)\s*,\s*([^,0-9]+?)\s*(\d+)/i;
  ...
  // Processa as linhas em ordem normal (do mais antigo para o mais recente)
  for (const linha of dados.slice(1)) {
    ...
    if (/Fim do primeiro tempo|Intervalo/i.test(texto)) {
      emIntervalo = true;
    } else if (/Início do segundo tempo/i.test(texto)) {
      emIntervalo = false;
    }
    ...
    if (/Fim do Jogo|Fim do segundo tempo/i.test(texto)) {
      ...
      // Extrai placar final diretamente
...
function exibirEventos(eventosAgrupados) {
  ...
  // Adiciona animação aos novos eventos
  const eventosElements = document.querySelectorAll(".evento-content");
