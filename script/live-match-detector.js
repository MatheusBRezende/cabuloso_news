...
function criarCardJogador(jogador) {
  ...
  const fotoHtml = fotoSrc
    ? `<img src="${escHtml(fotoSrc)}" alt="${escHtml(jogador.nome)}"
       ...
       <span style="display:none;width:100%;height:100%;border-radius:50%;align-items:center;justify-content:center;font-weight:700;font-size:1.1rem;background:#1a3a8f;color:#fff;">${(jogador.nome||'?').charAt(0).toUpperCase()}</span>`
    : `<span style="display:flex;width:100%;height:100%;border-radius:50%;align-items:center;justify-content:center;font-weight:700;font-size:1.1rem;background:#1a3a8f;color:#fff;">${(jogador.nome||'?').charAt(0).toUpperCase()}</span>`;
