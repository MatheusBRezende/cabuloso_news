...
function parseRelativeDate(str) {
  ...
  if (dataUOL) {
    const dia = Number.parseInt(dataUOL[1])
    const mes = Number.parseInt(dataUOL[2]) - 1
    const ano = Number.parseInt(dataUOL[3])
    const hora = dataUOL[4] ? Number.parseInt(dataUOL[4]) : 0
    const min = dataUOL[5] ? Number.parseInt(dataUOL[5]) : 0
  ...
  if (dataAbs) {
    ...
    const dia = Number.parseInt(dataAbs[1])
    const mes = meses.indexOf(dataAbs[2])
    const ano = Number.parseInt(dataAbs[3])
    if (mes >= 0) {
  ...
  if (str.includes("hora")) {
    const n = Number.parseInt(str)
    return isNaN(n) ? Number.POSITIVE_INFINITY : n * 60
  }
  if (str.includes("dia")) {
    const n = Number.parseInt(str)
    return isNaN(n) ? Number.POSITIVE_INFINITY : n * 24 * 60
  }
...
function cleanTeamName(teamName) {
  if (!teamName) return ""
  return teamName.replace(/^\d+°\s*/, "").replace(/\s[A-Z]{2,4}$/, "")
}
...
async function loadNextMatches() {
  ...
  try {
    ...
    for (let i = 1; i < data.values.length && count < 3; i++) {
      ...
      const dataJogo = new Date(hoje.getFullYear(), Number.parseInt(mes) - 1, Number.parseInt(dia))
      if (dataJogo < hoje) continue

      const isLive = row[7] === "LIVE" || row[7] === "AO VIVO"
      const colunaC = row[2] || ""

      // Tratamento aprimorado para jogos de ida e volta
      let faseInfo = ""
...
async function initWidgets() {
  ...
  if (apiKeyLoaded) {
    try {
      await Promise.all([loadMiniTable(), loadMiniResults(), loadNextMatches()])
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all([initWidgets(), fetchNews()])
    .then(() => {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible" && AppState.widgetsLoaded) {
      ...
      setTimeout(() => {
        Promise.all([loadMiniTable(), loadMiniResults(), loadNextMatches()])
      }, 1000)
    }
  })
